...
SVC_UUO:
        ...
        ANDC(r0,0xF,r0)         // Pick out low bits. Find step 2
...
SVCTbl: UUO(HaltH)              // SVC(0): User-mode HALT instruction
        ...
        UUO(MouseH)             // SVC(8): Retrieve information about last click
...
ProcTbl:
        ...
        LONG(ClickMsgStack)     // Process 3: SP
        LONG(ClickMsgStart)     // Process 3: XP (= PC)
...
.macro Mouse()  SVC(8)          // Read Mouse info. Find step 2
...
// Report click signal semaphore
        Signal(ClickS)
        LD(P0LinP, r3)          
        GetKey()                //read next char
        Wait(ClickS)            //then listen for click
        WrCh()
		CALL(UCase)
        ST(r0, 0, r3)           // store in buffer
        ADDC(r3, 4, r3)         // increment the pointer
        CMPEQC(r0, 0xA, r1)     // if we are at the end of the line
        BT(r1, P0Send)          // send what's in bounded buffer to process 1
...
////////////////////////////////////////////////////
/// Step 1: Mouse Interrupt Handler
////////////////////////////////////////////////////

Mouse_State: LONG(-1)

MouseH:               
    LD(Mouse_State, r0)         // return code in r0
    CMPEQC(r0, -1, r1)
    BNE(r1, I_Wait)             // wait if there hasn't been a click since last time
                                // got mouse info, return it and clear the mouse buffer
    LD(Mouse_State, r0)         // get mouse state
    ST(r0, UserMState)          // return it in r0
    CMOVE(-1, r1)
    ST(r1, Mouse_State)         // clear mouse buffer
    BR(I_Rtn)                   // return to user mode
...
////////////////////////////////////////////////////////////////////////
/// USER MODE Process 3: Send Mouse Click Message (Step 3)
////////////////////////////////////////////////////////////////////////
...
ClickMsgStart:
    Mouse()
    Wait(ClickS)   		// wait until ClickS=1
    LD(UserMState, r0)  // Load click info into ro
    ADD(r0, r31, r1)    // First save click info in r1
    SHRC(r0, 16, r0)    // Get x
    ST(r0, UserMState)
    WrMsg()             // Print x
    .text "\nClick at x="
	...
	ANDC(r1, 0xFF, r1)  // Get y
    ADDC(r1, 0, r0)		//ADD(r1, r31, r0)
    WrMsg()             // Print y
    .text ", y="
    HexPrt()

    Signal(ClickS)  // increment ClickS
//.breakpoint
    BR(ClickMsgStart)    // Start process all over again

ClickMsgStack: STORAGE(256)
